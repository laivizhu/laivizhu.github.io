...
var options=SAPO.extendObj({asynchronous:true,method:'POST',parameters:null,timeout:0,delay:0,postBody:'',contentType:'application/x-www-form-urlencoded',requestHeaders:null,onComplete:null,onSuccess:null,onFailure:null,onException:null,onHeaders:null,onCreate:null,onInit:null,onTimeout:null,sanitizeJSON:false,evalJS:true,xhrProxy:'',cors:false,debug:false},SAPO.Communication.Ajax.globalOptions);if(userOptions&&typeof userOptions=='object'){options=SAPO.extendObj(options,userOptions);if(typeof userOptions.parameters=='object'){options.parameters=SAPO.extendObj(SAPO.extendObj({},SAPO.Communication.Ajax.globalOptions.parameters),userOptions.parameters);}
...
(function(undefined){'use strict';SAPO.namespace('Ink');var Aux=SAPO.Ink.Aux,Css=SAPO.Dom.Css,Element=SAPO.Dom.Element,Event=SAPO.Dom.Event;var Modal=function(selector,options){this._options=SAPO.extendObj({width:undefined,height:undefined,markup:undefined,onShow:undefined,onDismiss:undefined,closeOnClick:false,skipDismiss:false,resize:false,disableScroll:false},options||{});this._handlers={click:this._onClick.bindObjEvent(this),keyDown:this._onKeyDown.bindObjEvent(this)};if(this._options.markup===undefined){this._referenceElement=Aux.elOrSelector(selector,'1st argument');}
this._wasDismissed=false;this._init();};Modal.prototype={_init:function(){var contentMarkup=this._options.markup!==undefined?this._options.markup:this._referenceElement.innerHTML;var s;var div2El=document.createElement('div');div2El.className='ink-modal ink-space';s=div2El.style;s.position='absolute';var w=600;var h=400;if(this._options.width){w=this._options.width;s.width=w+'px';}
if(this._options.height){h=this._options.height;s.height=h+'px';}
s.marginLeft=['-',~~(w/2),'px'].join('');s.marginTop=['-',~~(h/2),'px'].join('');s.left='50%';s.top='50%';div2El.innerHTML=[this._options.skipClose?'':'<a href="#" class="ink-close">×</a>',contentMarkup].join('');var divEl=document.createElement('div');divEl.className='ink-shade';s=divEl.style;s.position='fixed';s.left=0;s.right=0;s.top=0;s.bottom=0;divEl.appendChild(div2El);document.body.appendChild(divEl);this._contentElement=div2El;this._shadeElement=divEl;if(this._options.onShow){this._options.onShow(this);}
...
{var contentHeight=0;var curChildElm;for(var i=0,t=this._contentElement.childNodes.length;i<t;i++){curChildElm=this._contentElement.childNodes[i];if(curChildElm.nodeType===1){contentHeight+=SAPO.Dom.Element.elementHeight(curChildElm)||0;}}
if(contentHeight>SAPO.Dom.Element.elementHeight(this._contentElement)){this._contentElement.style.height=contentHeight+'px';var marginTop=~~(contentHeight/2);this._contentElement.style.marginTop=['-',marginTop,'px'].join('');var contentPos=SAPO.Dom.Element.offset2(this._contentElement);if(contentPos[1]<0){marginTop-=(Math.abs(contentPos[1])+20);contentHeight-=(Math.abs(contentPos[1])*2+30);this._contentElement.style.marginTop=['-',marginTop,'px'].join('');this._contentElement.style.height=contentHeight+'px';this._contentElement.style.overflow='auto';this._contentElement.style.overflowX='hidden';if(!this._options.skipClose){var aClose=SAPO.Dom.Selector.select('.ink-close',this._contentElement);if(aClose.length>0){aClose[0].style.top='5px';aClose[0].style.right='5px';}}}}},_disableScroll:function()
...
var el=this._shadeElement;this._wasDismissed=true;el.parentNode.removeChild(el);Aux.unregisterInstance(this._instanceId);},getContentElement:function(){return this._contentElement;},setContentMarkup:function(contentMarkup){this._contentElement.innerHTML=[this._options.skipDismiss?'':'<a href="#" class="ink-close">x</a>',contentMarkup].join('');}};Modal.destroy=Modal.dismiss;SAPO.Ink.Modal=Modal;})();
