...
function getDataDiv(item){
	var data="<li><h2>"+item.title+"</h2>"+"</br><hr>"+"<p>"+item.question+"</p>"+"</br><hr><p>"+item.content+"</p></br><hr color='red'></li>";
	return data;
}

function getJsonData(keyword,start){
	var result=$("#resultDivId");
	laivi.getJson('search_showResult.action?value='+keyword+'&start='+start+'&limit='+pagecount, function(data){
