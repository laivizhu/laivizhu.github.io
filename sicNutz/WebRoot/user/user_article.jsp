  ...
  <body>
	...
	<script type="text/javascript">
		var getMoreData=function(id){
			sic.common.getJson('../blog/article/get?fold=false&id='+id, function(result){
		...
		var getLessData=function(id){
			sic.common.getJson('../blog/article/get?fold=true&id='+id, function(result){
		...
		$(document).ready(sic.basic.init(function(){
            ...
            sic.pageLoding.pageLoad('../blog/article/list.action', $("#articleListDivId"), function(item){
                return "<div class='span8'><a href='../knowledge/article_view.jsp?id="+item.id+"'><h4>"+item.title+"</h4></a><p><div align='right'>"+item.createDate+" 作者："+item.user.name+"</div></p><p id='content"+item.id+"'>"+item.content+"<a class='btn btn-primary btn-small' onclick='getMoreData("+item.id+")'>More &raquo;</a></p><div align='right'><p><a class='btn' onclick=deleteObject('../blog/article/delete?id="+item.id+"')><i class='icon-remove-circle'></i>删除</a>|<a class='btn' href='user_addArticle.jsp?id="+item.id+"'><i class='icon-edit'></i>编辑</a></p></div></div>";
            });
		}));
