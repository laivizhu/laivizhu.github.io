...
public class ContractOperateAction extends ABasicAction<ContractBean> {
	...
	public String modifyView() throws Exception{
		try {
			...
			map.put("ehsRemark", ehsService.getList(conditions).get(0).getRemark());
			long id0=diviationHeadService.getList(conditions).get(0).getId();
			map.put("diviationId", String.valueOf(id0));
			ServletActionContext.getRequest().setAttribute(
					"contractConcent", map);
			...
			return AppConstants.ContractReturnConstants.SUCCESS_CONTRACT_MODIFYVIEW;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
